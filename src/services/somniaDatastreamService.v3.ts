...
class SomniaDatastreamServiceV3 {
  ...
  async getPostsPaginated(page: number = 0, pageSize: number = 20): Promise<PostDataV3[]> {
    ...
    try {
      ...
      deduplicatedData.sort((a, b) => {
        const extractTimestamp = (item: any) => {
          if (!item) return 0;
        ...
        // ✅ FIX: timestamp is at index [1], not [3]
        const timestampA = extractTimestamp(a[1]);
        const timestampB = extractTimestamp(b[1]);
        return timestampB - timestampA; // Newest first
      });

      console.log(`📅 [V3] Sorted ${deduplicatedData.length} posts by timestamp (newest first)`);

      // Debug: Log first 3 posts timestamps BEFORE parsing
      if (deduplicatedData.length > 0) {
        console.log('🔍 [V3-DEBUG] First 3 posts raw timestamps:', deduplicatedData.slice(0, 3).map((item, idx) => {
          const extractTimestamp = (val: any) => {
            if (!val) return 0;
            if (val.value !== undefined) {
              if (typeof val.value === 'object' && val.value.value !== undefined) {
          ...
          // ✅ FIX: timestamp is at index [1], not [3]
          const ts = extractTimestamp(item[1]);
          return {
            index: idx,
            timestamp: ts,
            date: new Date(ts * 1000).toLocaleString(),
            postId: String(extractTimestamp(item[0])).substring(0, 20)
          };
        }));
      }

      // Paginate merged data AFTER sorting
  ...
  async createPost(
    ...
    userWalletClient?: any,
    onOptimisticUpdate?: (postId: number) => void // ⚡ Callback for optimistic update
  ): Promise<number> {
    await this.ensureInitialized();
    ...
    try {
      ...
      // Generate ID immediately for optimistic update
      const timestamp = postData.timestamp || Date.now();
      const author = postData.author;
      const postId = postData.id || createPostId(author, timestamp);

      // ⚡ OPTIMISTIC UPDATE: Notify UI immediately
      if (onOptimisticUpdate) {
        console.log('⚡ [V3-OPTIMISTIC] Triggering optimistic update for post:', postId);
  ...
  async createInteraction(
    ...
    try {
      ...
      // ✅ Check if this is a follow/unfollow interaction for multi-publisher batching
      const isFollowInteraction =
        interactionData.interactionType === InteractionType.FOLLOW ||
        interactionData.interactionType === InteractionType.UNFOLLOW;

      // 🚀 SMART DETECTION: Check if we should use unified batch
      // Unified batch is used when multiple interaction types are pending
      const isBatchableInteraction = isLikeInteraction || isBookmarkInteraction || isRepostInteraction || isFollowInteraction;
  ...
  async getFollowerCount(userAddress: string): Promise<number> {
    try {
      ...
      console.log(`📊 [V3] Getting follower count for ${userAddress.slice(0, 10)}...`);
      console.log(`📊 [V3] Total interactions: ${interactions.length}`);

      // 🔍 DEBUG: Log all USER follow interactions to see what's in the data
      const allUserFollows = interactions.filter(
        i => i.targetType === TargetType.USER &&
          (i.interactionType === InteractionType.FOLLOW ||
            i.interactionType === InteractionType.UNFOLLOW)
      );
      console.log(`📊 [V3] Total USER follow/unfollow interactions: ${allUserFollows.length}`);
      if (allUserFollows.length > 0) {
        console.log(`📊 [V3] Sample follow interactions (first 3):`,
          allUserFollows.slice(0, 3).map(i => ({
            targetId: i.targetId,
            targetType: i.targetType,
            interactionType: i.interactionType === InteractionType.FOLLOW ? 'FOLLOW' : 'UNFOLLOW',
            fromUser: i.fromUser.slice(0, 10) + '...',
            content: i.content,
      ...
      // 🔍 DEBUG: Check if content matches the target address
      console.log(`📊 [V3] Looking for address: ${userAddress}`);
      const matchingByContent = allUserFollows.filter(
        i => i.content.toLowerCase() === userAddress.toLowerCase()
      );
      console.log(`📊 [V3] Interactions matching by content: ${matchingByContent.length}`);

      // ✅ FIX: Match by content (full address) instead of targetId
      // Because targetId conversion might be inconsistent between write and read
      const followInteractions = interactions.filter(
        i => i.content.toLowerCase() === userAddress.toLowerCase() &&
          i.targetType === TargetType.USER &&
          (i.interactionType === InteractionType.FOLLOW ||
      ...
      console.log(`📊 [V3] Found ${followInteractions.length} follow/unfollow interactions for address ${userAddress.slice(0, 10)}`);
      ...
      console.log(`✅ [V3] Follower count: ${count}`);
      return count;
    } catch (error) {
  ...
  async getFollowingCount(userAddress: string): Promise<number> {
    try {
      ...
      console.log(`📊 [V3] Getting following count for ${userAddress.slice(0, 10)}...`);
      console.log(`📊 [V3] Total interactions: ${interactions.length}`);
      ...
      console.log(`📊 [V3] Found ${followInteractions.length} follow/unfollow interactions by this user`);
      ...
      console.log(`✅ [V3] Following count: ${count}`);
      return count;
    } catch (error) {
  ...
  /**
   * ⚡ Subscribe to new posts in real-time using WebSocket
   * Returns unsubscribe function
   */
  async subscribeToNewPosts(onNewPost: (post: PostDataV3) => void): Promise<() => void> {
    await this.ensureInitialized();

    try {
      console.log('🔔 [V3-SUBSCRIBE] Setting up real-time subscription for new posts...');

      const postsSchemaId = await this.getSchemaIdCached('posts');

      // Subscribe to Somnia Datastream events
      const subscription = await this.sdk!.streams.subscribe({
        somniaStreamsEventId: 'PostCreated', // Event emitted when new post is created
        ethCalls: [], // No additional eth calls needed
        onData: async (data: any) => {
          try {
            console.log('🔔 [V3-SUBSCRIBE] New post detected!', data);

            // Parse the new post data from event
            // The event should contain the post ID or full post data
            // We'll need to fetch the full post data
            const postId = data.result?.topics?.[1]; // Assuming post ID is in topic1

            if (postId) {
              // Fetch the full post data
              const posts = await this.getAllPosts(1, 0); // Get latest post
              if (posts && posts.length > 0) {
                const newPost = posts[0];
                if (newPost) {
                  console.log('✅ [V3-SUBSCRIBE] New post fetched:', newPost.id);
                  onNewPost(newPost);
                }
              }
            }
          } catch (error) {
            console.error('❌ [V3-SUBSCRIBE] Error processing new post:', error);
          }
        },
        onError: (error: Error) => {
          console.error('❌ [V3-SUBSCRIBE] Subscription error:', error);
        },
        onlyPushChanges: true, // Only push when data changes
      });

      console.log('✅ [V3-SUBSCRIBE] Subscription active, listening for new posts...');

      // Return unsubscribe function
      return subscription?.unsubscribe || (() => {
        console.log('⚠️ [V3-SUBSCRIBE] No unsubscribe function available');
      });
    } catch (error) {
      console.error('❌ [V3-SUBSCRIBE] Failed to subscribe:', error);
