...
const AudioPlayer: React.FC = () => {
  ...
  const handlePlayPause = async () => {
    if (isPlaying) {
      ...
      console.log('🎵 [AudioPlayer] Play button clicked - attempting to resume');
      console.log('🎵 [AudioPlayer] User agent:', navigator.userAgent);
      console.log('🎵 [AudioPlayer] Is mobile:', /iPhone|iPad|iPod|Android/i.test(navigator.userAgent));
      
      setIsBuffering(true);
      try {
        // MOBILE FIX: Add a small delay to ensure user interaction is registered
  ...
  // Clear buffering state when playing starts
  useEffect(() => {
    if (isPlaying) {
  ...
  if (isMiniMode) {
    return (
      <div
        ref={miniPlayerRef}
        className="fixed z-30 bg-black/20 backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 shadow-2xl hover:shadow-3xl transition-all duration-300 max-w-md w-full cursor-move select-none overflow-hidden"
        style={{
          left: `${miniPosition.x}%`,
          top: `${miniPosition.y}%`,
  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 left-2 sm:left-4 right-2 sm:right-4 bg-black/20 backdrop-blur-xl border border-white/10 px-2 sm:px-4 py-2 sm:py-3 z-30 rounded-lg shadow-2xl mx-auto max-w-7xl overflow-hidden"
      style={{
    ...
    >
      ...
      {/* Mobile Layout */}
      <div className="relative z-10 flex flex-col gap-2 md:hidden">
        {/* Track Info + Controls */}
      ...
      <div className="relative z-10 hidden md:flex md:flex-row items-center gap-4">
        {/* Track Info */}
