...
import { TipTarget, tipService } from '@/services/tipService';
...
const PostCard: React.FC<PostCardProps> = ({
  ...
  const { balance } = useBalanceRefresh(smartAccountAddress);
  ...
  if (post) {
    ...
    if (post.contentType === 'video') {
      console.log('🎥 [VIDEO POST] Rendering:', {
        ...
        hasIpfsHash: !!post.ipfsHash,
        videoUrl: post.ipfsHash ? `https://ipfs.io/ipfs/${post.ipfsHash}` : 'NO HASH',
  ...
  return (
    <Card
      className="w-full border-0 border-b border-border/50 rounded-none hover:bg-white/[0.02] transition-colors duration-200 cursor-pointer bg-transparent"
    ...
    >
      ...
      <CardHeader className="pb-3 px-4 sm:px-6 pt-3 sm:pt-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center space-x-1 sm:space-x-2">
                <h3
                  className="font-semibold text-base truncate cursor-pointer hover:underline transition-all"
                  onClick={(e) => {
                    e.stopPropagation();
                    if (username) {
              ...
              <div className="flex items-center space-x-1 sm:space-x-2 text-sm text-muted-foreground">
                {username && (
                  <span
                    className="cursor-pointer hover:text-primary hover:underline transition-colors truncate max-w-[100px] sm:max-w-none"
                    onClick={(e) => {
                      e.stopPropagation();
                ...
                <div className="flex items-center space-x-0.5 sm:space-x-1">
                  ...
                  <span className="truncate">{formatTimeAgo}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <CardContent className="space-y-3 sm:space-y-4 px-4 sm:px-6 pb-4 sm:pb-5">
        ...
        {post.content && post.content.trim() && (
          <div className="space-y-2 cursor-pointer pl-[60px]">
        ...
        {post.contentType === 'music' && metadata && (
          <Card className="border-border/30 bg-white/[0.02] ml-[60px]">
            <CardContent className="p-2 sm:p-3">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                />
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-xs sm:text-sm truncate">{metadata.title || 'Untitled'}</h4>
                  <p className="text-[10px] sm:text-xs text-muted-foreground truncate">
                    {metadata.artist || 'Unknown Artist'}
                    {metadata.tokenId && (
                      <span className="ml-1 sm:ml-2 text-purple-600 hidden sm:inline">• NFT #{metadata.tokenId}</span>
                    )}
                  </p>
                  <div className="flex items-center gap-1 sm:gap-2 mt-0.5 sm:mt-1">
                    ...
                    <div className="flex-1 bg-muted rounded-full h-0.5 sm:h-1">
                      {(() => {
                        const isCurrentTrack = currentTrack?.id === (metadata.tokenId || Number(post.id));
                        ...
                        return (
                          <div className="bg-primary h-0.5 sm:h-1 rounded-full w-0"></div>
                        );
                      })()}
                    </div>
                  </div>
                  ...
                  onClick={(e) => {
                    ...
                    if (currentTrack?.id === track.id && audioIsPlaying) {
                      ...
                      const duration = typeof track.duration === 'string'
                        ? parseInt(track.duration.split(':')[0] || '0') * 60 + parseInt(track.duration.split(':')[1] || '0')
                        : track.duration || 180;
        ...
        {post.quotedPost && (
          <div
            className="mt-3 p-3 border border-border/30 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-colors cursor-pointer ml-[60px]"
            onClick={(e) => {
              e.stopPropagation();
        ...
        {post.ipfsHash && (post.contentType === 'image' || post.contentType === 'quote') && (
          <img
            ...
            alt="Post"
            className={`rounded-lg max-h-96 object-cover ml-[60px] max-w-[calc(100%-60px)] ${post.quotedPost ? 'mt-3' : ''}`}
            loading="lazy"
            decoding="async"
            onError={(e) => {
              const target = e.target as HTMLImageElement;
        ...
        {post.ipfsHash && post.contentType === 'video' && (
          <div className={`ml-[60px] max-w-[calc(100%-60px)] ${post.quotedPost ? 'mt-3' : ''}`}>
            <div className="relative rounded-2xl overflow-hidden bg-black border border-border/20" style={{ maxHeight: '380px', maxWidth: '100%' }}>
              <video
                src={`https://ipfs.io/ipfs/${post.ipfsHash}`}
                controls
        ...
        {metadata?.attachments && metadata.attachments.length > 0 && (
          <div className={`mt-3 ${metadata.attachments.length === 1
        ...
        <TooltipProvider delayDuration={300}>
          <div className="flex items-center justify-between pt-3 pl-[60px]">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className={`h-7 sm:h-8 px-1.5 sm:px-2 space-x-1 sm:space-x-2 transition-all duration-200 ${isLiked ? 'text-red-500' : 'text-muted-foreground hover:text-red-500'} hover:bg-red-500/10`}
                    onClick={(e) => {
                      e.stopPropagation();
                      handleLike();
                    }}
                    disabled={isLoading}
                  >
                    <Heart className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-all duration-200 ${isLiked ? 'fill-current' : ''} ${isAnimating ? 'heart-burst' : isLiked ? 'heart-pop' : ''}`} />
                    <span className="text-[10px] sm:text-xs transition-all duration-200">{likesCount}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent 
                  side="top" 
                  className="bg-slate-700 text-white border-0 px-3 py-2 text-sm font-medium"
                >
                  {isLiked ? 'Unlike' : 'Like'}
                </TooltipContent>
              </Tooltip>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 sm:h-8 px-1.5 sm:px-2 space-x-1 sm:space-x-2 text-muted-foreground hover:text-blue-500 hover:bg-blue-500/10"
                  onClick={(e) => {
                    e.stopPropagation();
                    handleComment();
                ...
                >
                  ...
                  <span className="text-[10px] sm:text-xs">{post?.comments || 0}</span>
                </Button>
              </TooltipTrigger>
              <TooltipContent 
                side="top" 
                  ...
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-7 sm:h-8 px-2 sm:px-3 text-muted-foreground hover:text-green-500 hover:bg-green-500/10 transition-colors gap-0.5 sm:gap-1"
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                        disabled={isLoading}
                      >
                        ...
                        <span className="text-[10px] sm:text-xs hidden sm:inline">Tip</span>
                      </Button>
                    </DropdownMenuTrigger>
                  </TooltipTrigger>
                  <DropdownMenuContent 
                  ...
                  >
                    <div className="space-y-3">
                      <div className="text-sm text-muted-foreground">
                        Balance: {balance?.formatted ? parseFloat(balance.formatted).toFixed(2) : '0.00'} STT
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[
                          ...
                          <Button
                            ...
                            className="h-7 rounded-full text-xs py-0.5 px-3 min-h-0"
                            onClick={(e) => {
                              e.stopPropagation();
                      ...
                      <Button
                        ...
                        onClick={async (e) => {
                          ...
                          try {
                            const amountInWei = tipService.parseTipAmount(amount);
                            
                            toast.loading(`Sending ${amount} STT tip...`);
                            ...
                            toast.success(`${amount} STT tip sent successfully!`);
                            setAmount(''); // Reset amount after successful tip - user must select again
                          } catch (error: any) {
            ...
            <Tooltip>
                ...
                <TooltipTrigger asChild>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="sm"
                      className={`h-7 sm:h-8 px-1.5 sm:px-3 gap-0.5 sm:gap-1 ${isReposted ? 'text-green-500' : 'text-muted-foreground hover:text-green-500'} hover:bg-green-500/10`}
                      onClick={(e) => {
                        e.stopPropagation();
                      }}
                      disabled={isLoading}
                    >
                      <Repeat2 className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isReposted ? 'text-green-500' : ''}`} />
                      <span className="text-[10px] sm:text-xs hidden sm:inline">Repost</span>
                    </Button>
                  </DropdownMenuTrigger>
                </TooltipTrigger>
              ...
              <TooltipContent 
                ...
                className="bg-slate-700 text-white border-0 px-3 py-2 text-sm font-medium"
              >
                {sharesCount} {sharesCount === 1 ? 'Repost' : 'Reposts'}
              </TooltipContent>
            </Tooltip>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    className={`h-7 sm:h-8 px-1.5 sm:px-2 transition-all duration-200 ${localBookmarkState ? 'text-blue-500' : 'text-muted-foreground hover:text-blue-500'} hover:bg-blue-500/10`}
                    onClick={(e) => {
                      e.stopPropagation();
                      handleBookmark();
                    }}
                    disabled={isLoading}
                  >
                    <Bookmark className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-all duration-200 ${localBookmarkState ? 'fill-current' : ''} ${isBookmarkAnimating ? 'bookmark-pop' : localBookmarkState ? 'bookmark-saved' : ''}`} />
                  </Button>
                </TooltipTrigger>
                <TooltipContent 
                  side="top" 
                  className="bg-slate-700 text-white border-0 px-3 py-2 text-sm font-medium"
                >
                  {localBookmarkState ? 'Remove bookmark' : 'Bookmark'}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
        ...
        {/* View Likes and Reposts Links - Hidden for cleaner UI */}
        <div className="hidden flex items-center gap-3 sm:gap-4 pt-2">
          {likesCount > 0 && onViewLikes && (
            <button
              ...
              className="text-[10px] sm:text-xs text-muted-foreground hover:underline hover:text-foreground transition-colors"
            >
