...
import { Music, RefreshCw, Play, CheckCircle, Clock, ChevronLeft, ChevronRight, Coins, Loader2, Upload, Sparkles } from 'lucide-react';
...
export const GeneratedMusicHistory = () => {
  ...
  const handleMintNFT = async (song: GeneratedMusicData) => {
    ...
    try {
      ...
      if (song.audioUrl && song.audioUrl.includes('ipfs://')) {
        ipfsAudioHash = song.audioUrl.replace('ipfs://', '') || '';
      } else if (song.audioUrl && song.audioUrl.includes('/ipfs/')) {
        const match = song.audioUrl.match(/\/ipfs\/([^/?]+)/);
        if (match) ipfsAudioHash = match[1] || '';
      }

      if (song.imageUrl && song.imageUrl.includes('ipfs://')) {
        ipfsImageHash = song.imageUrl.replace('ipfs://', '') || '';
      } else if (song.imageUrl && song.imageUrl.includes('/ipfs/')) {
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-0">
          ...
          >
            <RefreshCw className={`w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 ${isRefreshing ? 'animate-spin' : ''}`} />
            Refresh
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent>
        ...
        {datastreamMusic.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            >
              ...
              <span className="sm:hidden">AI</span> ({datastreamMusic.filter(s => s.status === GeneratedMusicStatus.COMPLETED && !s.taskId?.startsWith('upload-')).length})
            </Button>
            <Button
        ...
        <div className="space-y-3">
            {isLoading ? (
                  ...
                  <CardContent className="p-3 md:p-4">
                    <div className="flex items-start gap-2 md:gap-3">
                      <img
                        ...
                        alt={song.title}
                        className="w-12 h-12 md:w-16 md:h-16 rounded-lg object-cover shadow-sm flex-shrink-0"
                        onError={(e) => {
                        ...
                        <div className="flex items-center gap-1 md:gap-2">
                          ...
                          {song.taskId?.startsWith('upload-') ? (
                            <div title="Uploaded Song">
                              <Upload className="w-3 h-3 md:w-3.5 md:h-3.5 text-blue-500 flex-shrink-0" />
                            </div>
                          ) : (
                            <div title="AI Generated">
                              <Sparkles className="w-3 h-3 md:w-3.5 md:h-3.5 text-purple-500 flex-shrink-0" />
                            </div>
                          )}
                        </div>
                        <p className="text-[10px] md:text-xs text-muted-foreground truncate mt-0.5">{song.style}</p>
                      ...
                      <div className="flex flex-col gap-1 md:gap-2">
                        ...
                        {song.status === GeneratedMusicStatus.COMPLETED && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="text-[10px] md:text-xs px-1.5 md:px-2 py-1 h-auto gap-0.5 md:gap-1"
                          ...
                          >
                            {mintingMusicId === song.id ? (
                              <>
                                ...
                                <span className="hidden md:inline">Minting...</span>
                              </>
                            ) : (
                              <>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex flex-col md:flex-row items-center justify-between gap-3 mt-4 md:mt-6 pt-4 border-t">
                  ...
                  <div className="flex gap-1 md:gap-2">
                    <Button
                      ...
                      className="text-xs md:text-sm h-8 md:h-9"
                    >
                      <ChevronLeft className="w-3 h-3 md:w-4 md:h-4 mr-0 md:mr-1" />
                    ...
                    >
                      ...
                      <ChevronRight className="w-3 h-3 md:w-4 md:h-4 ml-0 md:ml-1" />
                    </Button>
                  </div>
                </div>
              )}
