        ...
        <div className="relative container mx-auto px-6 py-8 min-h-[400px] flex items-center">
          <div className="flex flex-col md:flex-row gap-8 items-start w-full">
            ...
            <div className="flex-1 space-y-4">
              ...
              {/* Followed by (Mutual Followers) */}
              {(() => {
                // Mock mutual followers - replace with real data from useMutualFollowers hook
                const mutualFollowers = [
                  { username: 'sealaunch.xyz', displayName: 'SeaLaunch', initial: 'S' },
                  { username: 'mp', displayName: 'MP', initial: 'M' }
                ];
                ...
                return (
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      Followed by{' '}
                      {mutualFollowers.length === 1 && mutualFollowers[0] && (
                        <span className="text-foreground font-medium">{mutualFollowers[0].username}</span>
                      )}
                      {mutualFollowers.length === 2 && mutualFollowers[0] && mutualFollowers[1] && (
                        <>
                          <span className="text-foreground font-medium">{mutualFollowers[0].username}</span>
                          {' '}and{' '}
                          <span className="text-foreground font-medium">{mutualFollowers[1].username}</span>
                        </>
                      )}
                      {mutualFollowers.length > 2 && mutualFollowers[0] && mutualFollowers[1] && (
                        <>
                          ...
                          {', '}
                          <span className="text-foreground font-medium">{mutualFollowers[1].username}</span>
                          {' '}and {mutualFollowers.length - 2} other{mutualFollowers.length - 2 > 1 ? 's' : ''}
                        </>
                      )}
                    </span>
                  </div>
