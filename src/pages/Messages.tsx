...
export default function Messages() {
  ...
  return (
    <div className="min-h-screen bg-black">
      <Navbar 
        showCreateSongButton={true} 
        hideBottomNav={!!(selectedConversation && selectedRecipient)}
        hideOnMobile={!!(selectedConversation && selectedRecipient)}
      />
      
      {/* Main Content */}
      <main className={`${
        selectedConversation && selectedRecipient 
          ? 'pt-0 md:pt-16' // No padding on mobile when in chat
          : 'pt-16 md:pt-16' // Normal padding when showing navbar
      }`}>
        <div className="container mx-auto px-0 md:px-6 py-0 md:py-3">
          {/* Header - Desktop Only */}
          <div className="mb-3 px-4 md:px-0 pt-3 md:pt-0 hidden md:block">
          ...
          <div className={`flex gap-0 md:gap-6 ${
            selectedConversation && selectedRecipient 
              ? 'h-screen md:h-[calc(100vh-140px)]' // Full screen on mobile when in chat (navbar & bottom nav hidden)
              : 'h-[calc(100vh-64px-64px)] md:h-[calc(100vh-140px)]' // Reduced height when showing navbar & bottom nav
          }`}>
            {/* Sidebar - Conversation List */}
            {/* Mobile: Full screen when no conversation selected, hidden when conversation selected */}
            {/* Desktop: Always visible sidebar */}
            <div className={`${
              selectedConversation && selectedRecipient ? 'hidden md:flex' : 'flex'
            } w-full md:w-[340px] bg-background md:bg-card border-0 md:border md:border-border md:rounded-xl flex-col overflow-hidden`}>
              {/* Mobile Header */}
              <div className="md:hidden px-4 py-3 border-b border-border bg-card">
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="font-clash font-semibold text-xl">Messages</h1>
                  <span className="px-1.5 py-0.5 bg-primary/10 text-primary text-[9px] font-medium rounded-full border border-primary/20">Live</span>
                </div>
              </div>

              {/* Conversation List */}
              <div className="p-0 md:p-4 border-0 md:border-b md:border-border flex-1 overflow-hidden">
                <ConversationList
            ...
            {/* Main Chat Area */}
            {/* Mobile: Full screen when conversation selected, hidden when no conversation */}
            {/* Desktop: Always visible, shows placeholder when no conversation */}
            <div className={`${
              selectedConversation && selectedRecipient ? 'flex' : 'hidden md:flex'
            } flex-1 bg-background md:bg-card border-0 md:border md:border-border md:rounded-xl flex-col overflow-hidden`}>
              {(() => {
                ...
                return selectedConversation && selectedRecipient ? (
                  <ChatWindow
                    ...
                    onBack={() => {
                      ...
                      console.log('⬅️ [MESSAGES PAGE] Back button clicked')
