...
const InteractionLogs = () => {
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="container mx-auto px-3 sm:px-4 pt-20 sm:pt-24 pb-20 sm:pb-6 max-w-7xl">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" size="sm" onClick={loadLogs} className="flex-1 sm:flex-none">
            ...
            <Button variant="destructive" size="sm" onClick={handleClear} className="flex-1 sm:flex-none">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
              ...
              <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-green-500">{stats.success}</p>
                  </div>
              ...
              <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xl sm:text-2xl font-bold text-red-500">{stats.failed}</p>
                  </div>
            ...
            <Card>
              <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs sm:text-sm text-muted-foreground">User Wallet</p>
                    <p className="text-xl sm:text-2xl font-bold text-blue-500">{stats.userWallet}</p>
                  </div>
        ...
        <Card className="mb-6">
          <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4">
              ...
              <div className="w-full sm:w-auto">
                <p className="text-xs sm:text-sm font-medium mb-2">Wallet</p>
                <div className="flex flex-wrap gap-2">
                  <Button
          ...
          <CardContent className="px-3 sm:px-6">
            {filteredLogs.length === 0 ? (
              <div className="text-center py-8 sm:py-12 text-muted-foreground">
                <Activity className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 opacity-50" />
              ...
              <div className="space-y-2 sm:space-y-3 max-h-[500px] sm:max-h-[600px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
                {filteredLogs.reverse().map((log) => (
                  <Card key={log.timestamp} className="border-l-2 sm:border-l-4" style={{
                          ...
                          <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                            {log.data.fromUser && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">From:</span>
                                <code className="text-xs bg-muted px-1.5 sm:px-2 py-0.5 sm:py-1 rounded overflow-x-auto max-w-full">
                                  {log.data.fromUser.slice(0, 8)}...{log.data.fromUser.slice(-6)}
                                </code>
                              </div>
                            )}
                            ...
                            {log.data.author && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">Author:</span>
                                <code className="text-xs bg-muted px-1.5 sm:px-2 py-0.5 sm:py-1 rounded overflow-x-auto max-w-full">
                                  {log.data.author.slice(0, 8)}...{log.data.author.slice(-6)}
                                </code>
                              </div>
                            )}
                            ...
                            {log.data.targetId && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">Target:</span>
                                <code className="text-xs bg-muted px-1.5 sm:px-2 py-0.5 sm:py-1 rounded">
                                  {log.data.targetId}
                                </code>
                              </div>
                            )}
                            ...
                            {log.data.content && (
                              <div className="flex gap-2 items-start">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">Content:</span>
                                <span className="text-xs break-words">
                                  {log.data.content.substring(0, 40)}
                                  {log.data.content.length > 40 && '...'}
                                </span>
                              </div>
                            )}
                            ...
                            {log.data.recipientAddress && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">To:</span>
                                <code className="text-xs bg-muted px-1.5 sm:px-2 py-0.5 sm:py-1 rounded overflow-x-auto max-w-full">
                                  {log.data.recipientAddress.slice(0, 8)}...{log.data.recipientAddress.slice(-6)}
                                </code>
                              </div>
                            )}
                            ...
                            {log.data.txHash && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">TX:</span>
                                <code className="text-xs bg-green-500/10 text-green-600 dark:text-green-400 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-green-500/20 overflow-x-auto max-w-full">
                                  {log.data.txHash.slice(0, 8)}...{log.data.txHash.slice(-6)}
                                </code>
                              </div>
                            )}
                            ...
                            {log.data.publisherAddress && (
                              <div className="flex gap-2 items-center">
                                <span className="text-muted-foreground flex-shrink-0 text-xs">Publisher:</span>
                                <code className="text-xs bg-blue-500/10 text-blue-600 dark:text-blue-400 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-blue-500/20 overflow-x-auto max-w-full">
                                  {log.data.publisherAddress.slice(0, 8)}...{log.data.publisherAddress.slice(-6)}
                                </code>
                              </div>
                            )}
                            ...
                            {log.type === 'BATCH_FLUSH_UNIFIED' && log.data.rawData && (
                              <div className="mt-2 p-2 sm:p-3 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded border sm:border-2 border-purple-500/40">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-2 flex-wrap">
                                  <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500" />
                                  <p className="text-xs sm:text-sm text-purple-600 dark:text-purple-400 font-bold">
                                    🚀 UNIFIED BATCH FLUSH
                                  </p>
                                  ...
                                  <div className="mt-2">
                                    ...
                                    <div className="space-y-1">
                                      {log.data.rawData.allTxHashes.map((tx: any, idx: number) => (
                                        <div key={idx} className="flex items-center gap-1.5 sm:gap-2 text-xs flex-wrap">
                                          <code className="bg-green-500/10 text-green-600 dark:text-green-400 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded border border-green-500/20 text-xs overflow-x-auto">
                            ...
                            {log.type === 'BATCH_FLUSH_LIKE' && log.data.rawData && (
                              <div className="mt-2 p-2 sm:p-3 bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 rounded border sm:border-2 border-red-500/40">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-2 flex-wrap">
                                  <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-red-500" />
                                  <p className="text-xs sm:text-sm text-red-600 dark:text-red-400 font-bold">
                                    ❤️ LIKE BATCH FLUSH
                                  </p>
                            ...
                            {log.type === 'BATCH_FLUSH_BOOKMARK' && log.data.rawData && (
                              <div className="mt-2 p-2 sm:p-3 bg-gradient-to-r from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20 rounded border sm:border-2 border-yellow-500/40">
                                <div className="flex items-center gap-1.5 sm:gap-2 mb-2 flex-wrap">
                                  <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-600" />
                                  <p className="text-xs sm:text-sm text-yellow-700 dark:text-yellow-400 font-bold">
            ...
            <CardContent className="px-3 sm:px-6">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4">
                {Object.entries(stats.byType).map(([type, count]: [string, any]) => (
                  count > 0 && (
                    <div key={type} className="p-3 sm:p-4 bg-muted rounded-lg">
                      <p className="text-xs sm:text-sm text-muted-foreground truncate" title={type}>{type}</p>
