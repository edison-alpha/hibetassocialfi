...
const DetailAlbum = () => {
  ...
  const getAlbumTypeLabel = (type: string | number) => {
    ...
    return types[type] || types[typeStr] || 'Album';
  };
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      {/* iOS-style Back Button - Fixed at top */}
      <div className="fixed top-16 left-0 right-0 z-40 bg-background/80 backdrop-blur-xl border-b border-border/10 md:hidden">
        <div className="px-4 py-2">
          <Button 
            variant="ghost" 
            size="sm" 
            onClick={() => navigate(-1)} 
            className="gap-1 text-primary hover:bg-primary/10 -ml-2"
          >
      ...
      {/* Desktop Back Button */}
      <div className="hidden md:block pt-16 pb-4">
        <div className="container mx-auto px-6">
      ...
      {/* Album Header - iOS Style */}
      <div className="relative overflow-hidden md:border-b md:border-border/20 pt-[52px] md:pt-0 min-h-[380px] md:min-h-[400px]">
        {/* iOS-style Gradient Background */}
        <div className="absolute inset-0 bg-gradient-to-b from-muted/30 via-background/95 to-background"></div>

        {/* Blurred Album Art Background (iOS Music App style) */}
        {album.coverImageHash && (
          <>
            <div
              className="absolute inset-0 bg-cover bg-center scale-110 blur-3xl opacity-20"
              style={{ 
                backgroundImage: `url(${getIPFSUrl(album.coverImageHash)})`
              }}
            ></div>
        ...
        <div className="relative container mx-auto px-4 md:px-6 py-6 md:py-8 min-h-[380px] md:min-h-[400px] flex items-center">
          <div className="flex flex-col md:flex-row gap-5 md:gap-8 items-center md:items-start w-full">
            {/* Album Cover - iOS Style with Shadow */}
            <div className="relative flex-shrink-0">
              <div className="w-48 h-48 md:w-56 md:h-56 rounded-2xl overflow-hidden shadow-2xl bg-muted/50 backdrop-blur-sm border border-white/10">
                {album.coverImageHash ? (
                  <img
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <Music className="w-12 h-12 md:w-16 md:h-16 text-muted-foreground/20" />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 space-y-3 md:space-y-4 w-full">
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h1 className="font-clash font-semibold text-xl sm:text-2xl md:text-4xl text-foreground">{album.title || 'Untitled Album'}</h1>
                  {album.albumType && (
                ...
                <Link 
                  to={`/profile/${artistProfile?.username || (typeof album.artist === 'object' ? album.artist.username : '') || (typeof album.artist === 'string' ? album.artist : album.artist?.id)}`}
                  className="text-muted-foreground text-sm md:text-lg hover:text-primary transition-colors"
                >
                  by {artistProfile?.displayName || (typeof album.artist === 'object' ? (album.artist.displayName || album.artist.username) : '') || (typeof album.artist === 'string' ? `${album.artist.slice(0, 6)}...${album.artist.slice(-4)}` : `${album.artist?.id?.slice(0, 6)}...${album.artist?.id?.slice(-4)}`)}
                </Link>
              </div>
              ...
              <div className="flex gap-4 md:gap-6 text-sm">
                <div className="text-center">
                  <p className="font-semibold text-lg md:text-xl text-foreground">{albumTracks.length}</p>
                  <p className="text-xs md:text-sm text-muted-foreground">Tracks</p>
                </div>
                <div className="text-center">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground">Plays</p>
                </div>
              </div>
              ...
              <div className="space-y-2 hidden md:block">
                {album.description && album.description.trim() && (
                  <p className="text-sm leading-relaxed max-w-2xl text-muted-foreground line-clamp-2">{album.description}</p>
                )}
              ...
              <div className="flex flex-wrap gap-2 md:gap-3">
                ...
                {address && (typeof album.artist === 'string' ? album.artist : album.artist.id).toLowerCase() === address.toLowerCase() && (
                  <>
                    ...
                    {(() => {
                      const albumType = String(album.albumType).toUpperCase();
                      ...
                      return canAddMore ? (
                        <Button
                          ...
                          size="sm"
                          className="px-4 md:px-8 shadow-lg bg-primary hover:bg-primary/90 shadow-primary/25"
                        >
                ...
                <Button
                  ...
                  size="sm"
                  className={`px-4 md:px-8 shadow-lg ${isFollowing ? 'bg-muted text-muted-foreground hover:bg-muted' : 'bg-primary hover:bg-primary/90 shadow-primary/25'}`}
                >
                  <span className="text-xs md:text-sm">{isFollowing ? 'Following' : 'Follow'}</span>
                </Button>
                <LikeAlbumButton albumId={parseInt(albumId!)} />
                <Button variant="outline" size="sm" className="border-border/50 hover:bg-muted/50 px-2 md:px-3">
                  <Share2 className="w-4 h-4" />
                </Button>
                {/* Support Album Button */}
                {album && (typeof album.artist === 'string' ? album.artist : album.artist?.id) !== address?.toLowerCase() && (
                  <TipButton
                    targetType={TipTarget.ALBUM}
                    targetId={parseInt(albumId!)}
                    recipientAddress={typeof album.artist === 'string' ? album.artist : album.artist.id}
                    recipientName={artistProfile?.displayName || (typeof album.artist === 'object' ? album.artist.displayName || album.artist.username : '')}
                    ...
                    showIcon={true}
                    showText={false}
                    className="border-green-500/50 text-green-500 hover:bg-green-500/10 px-2 md:px-3"
                  />
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-3 md:space-y-4">
            <div className="flex items-center justify-between mb-4 md:mb-6">
              <h2 className="font-clash font-semibold text-xl md:text-2xl">Tracks</h2>
              {albumTracks.length > 0 && (
                <Badge variant="outline" className="text-xs md:text-sm">
                  <span className="hidden sm:inline">{albumTracks.length} tracks • {formatDuration(albumTracks.reduce((sum, track) => sum + track.duration, 0))}</span>
                  <span className="sm:hidden">{albumTracks.length}</span>
                </Badge>
              )}
            </div>
            ...
            {bestSong && albumTracks.length > 1 && (
              <div className="mb-3 md:mb-4 p-3 md:p-4 rounded-lg bg-gradient-to-r from-primary/10 to-secondary/10 border border-primary/20">
                <div className="flex items-center gap-2 md:gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-xs md:text-sm mb-1">🏆 Best Song in Album</h3>
            ...
            {/* Track List */}
            <div className="space-y-1.5 md:space-y-2 max-h-[600px] overflow-y-auto pr-1 md:pr-2 scrollbar-thin">
              {albumTracks.length > 0 ? (
                albumTracks.map((track, index) => {
                  ...
                  <div
                    key={track.tokenId}
                    className={`group flex items-center gap-2 md:gap-4 p-2 md:p-4 rounded-lg hover:bg-muted/50 transition-colors duration-200 cursor-pointer border ${
                      isBestSong 
                  ...
                  >
                    {/* Track Number */}
                    <div className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center text-xs md:text-sm font-medium text-muted-foreground group-hover:text-foreground flex-shrink-0">
                      {isBestSong ? <Award className="w-3 h-3 md:w-4 md:h-4 text-primary" /> : index + 1}
                    </div>

                    {/* Track Cover with Play Button Overlay */}
                    <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-md overflow-hidden flex-shrink-0 bg-muted">
                      {/* Cover Image */}
                      {track.ipfsArtworkHash ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-muted">
                          <Music className="w-4 h-4 md:w-5 md:h-5 text-muted-foreground/30" />
                        </div>
                      )}
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <Button
                          size="sm"
                          variant="ghost"
                          className="w-6 h-6 md:w-8 md:h-8 p-0 text-white hover:bg-white/20"
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 md:gap-2 mb-0.5 md:mb-1">
                        <h3 className="font-medium text-xs md:text-sm truncate">{track.title}</h3>
                    ...
                    <div className="flex items-center gap-1 md:gap-3 flex-shrink-0">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="w-6 h-6 md:w-8 md:h-8 p-0 opacity-0 group-hover:opacity-100 transition-opacity hidden md:flex"
                      >
                ...
                <div className="text-center py-12 md:py-16 text-muted-foreground">
                  <Music className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 md:mb-4 opacity-20" />
                  <p className="text-base md:text-lg font-semibold mb-1 md:mb-2">No tracks yet</p>
                  <p className="text-xs md:text-sm">This album doesn't have any tracks added yet.</p>
                </div>
              )}
            </div>
          </div>
            ...
            */}
            ...
            <Card className="border-border/50">
              <CardContent className="p-4 md:p-6">
                <h3 className="font-clash font-semibold text-base md:text-lg mb-3 md:mb-4">Album Details</h3>
            ...
            <Card className="border-border/50">
              <CardContent className="p-4 md:p-6">
                <h3 className="font-clash font-semibold text-base md:text-lg mb-3 md:mb-4">Supporters</h3>
                ...
                {!isLoadingTips && !isLoadingSupporters && supportersWithProfiles.length === 0 && (
                  <div className="text-center py-6 md:py-8 text-muted-foreground">
                    ...
                    <p className="text-xs md:text-sm">No supporters yet</p>
                    ...
                    {supportersWithProfiles.map((supporter, index) => {
                      ...
                      return (
                        <div key={supporter.address} className="relative group">
                          {/* Profile Preview Card - Hidden on mobile */}
                          <div className="hidden md:block absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                          ...
                          {/* Avatar */}
                          <Link to={`/profile/${supporter.username || supporter.address}`}>
                            <div className="flex flex-col items-center gap-1 md:gap-2 group/avatar">
                              <div className="relative">
                                <Avatar className="w-10 h-10 md:w-12 md:h-12 ring-2 ring-primary/20 hover:ring-primary/40 transition-all duration-200 group-hover:scale-105">
                                  <AvatarImage src={avatarUrl} />
                                ...
                                {index < 3 && (
                                  <div className="absolute -top-1 -right-1 w-4 h-4 md:w-5 md:h-5 bg-primary rounded-full flex items-center justify-center shadow-lg">
                                    <span className="text-xs font-semibold text-primary-foreground">{index + 1}</span>
                                  </div>
                                )}
                              </div>
                              <div className="text-center opacity-0 md:group-hover/avatar:opacity-100 transition-opacity duration-200 hidden md:block">
            ...
            <Card className="border-border/50">
              <CardContent className="p-4 md:p-6">
                <h3 className="font-clash font-semibold text-base md:text-lg mb-2 md:mb-3">Tags</h3>
                <div className="flex flex-wrap gap-2">
                  ...
                  {(() => {
                    ...
                    const albumTypeLabel = getAlbumTypeLabel(album?.albumType || 'ALBUM');
                    if (albumTypeLabel) {
