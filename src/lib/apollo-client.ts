...
const SUBGRAPH_ENDPOINT = import.meta.env.VITE_SUBGRAPH_ENDPOINT || 'https://api.subgraph.somnia.network/api/public/801a9dbd-5ca8-40a3-bf29-5309f9d3177c/subgraphs/hibeats-social-subgraph/v4.5.0/gn';

console.log('🔗 [Apollo] Initializing with endpoint:', SUBGRAPH_ENDPOINT);
console.log('🔗 [Apollo] Using environment variable:', import.meta.env.VITE_SUBGRAPH_ENDPOINT ? 'Yes' : 'No (using default)');
...
export const apolloClient = new ApolloClient({
  ...
  defaultOptions: {
    watchQuery: {
      fetchPolicy: 'cache-and-network', // ✅ Get cache first, then update with network
      nextFetchPolicy: 'cache-first',   // ✅ Subsequent fetches use cache
      errorPolicy: 'all',
    },
    query: {
      fetchPolicy: 'cache-and-network', // ✅ Show cache immediately, then update
      errorPolicy: 'all',
    },
  },
});
